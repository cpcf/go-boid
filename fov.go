...
func (v *View) Compute(opaque func(x, y int) bool, px, py, radius int) {
	v.Visible = make(map[point]struct{})
	v.Visible[point{px, py}] = struct{}{}
	for i := 1; i <= 8; i++ {
		v.fov(opaque, px, py, 1, 0, 1, i, radius)
	}
}
...
func (v *View) fov(opaque func(x, y int) bool, px, py, dist int, lowSlope, highSlope float64, oct, rad int) {
	...
	for height := low; height <= high; height++ {
		...
		if opaque(mapx, mapy) {
			if inGap {
				...
				v.fov(opaque, px, py, dist+1, lowSlope, (height-0.5)/float64(dist), oct, rad)
			}
			...
			if height == high {
				v.fov(opaque, px, py, dist+1, lowSlope, highSlope, oct, rad)
			}
		}
	}
}
